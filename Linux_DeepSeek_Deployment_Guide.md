# Wren AI Linux + DeepSeek 本机部署使用手册

本文档适用于在 Linux 本机使用 Docker Compose 部署当前 `legacy/v1` 分支，并使用 DeepSeek 作为 LLM。
...
- Docker
- Docker Compose v2
- 可访问 `ghcr.io`、`qdrant/qdrant` 等镜像源
- DeepSeek API Key
...
```bash
cp ../wren-ai-service/docs/config_examples/config.deepseek.yaml config.yaml
```
...
如果 `wren-ui` 未正常运行，查看日志：

```bash
docker compose --env-file .env logs -f wren-ui
```
